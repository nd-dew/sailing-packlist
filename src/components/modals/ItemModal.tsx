...
export const ItemModal: React.FC = () => {
  const { 
    ...
    toggleCheck, hideItem, cycleLuggage, getNextLuggageHint, hiddenItems, unhideItem
  } = usePacklist();
            ...
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, marginRight: '10px' }}>
              ...
              <div className="modal-title-wrapper">
                <input 
                  ...
                  style={{ textDecoration: isHidden ? 'line-through' : 'none', opacity: isHidden ? 0.6 : 1 }}
                />
          ...
          <div className="modal-body">
            ...
            {showAdvanced && (
              <div className="modal-advanced-content">
                ...
                <div className="modal-field" style={{ marginTop: '20px', borderTop: '1px solid var(--border)', paddingTop: '20px', display: 'flex', gap: '10px' }}>
                  <button 
                    onClick={() => isHidden ? unhideItem(selectedItem.id) : hideItem(selectedItem.id)}
                    style={{ flex: 1, padding: '10px', background: isHidden ? '#e6f2ff' : '#f0f0f0', border: isHidden ? '1px solid #b3d4fc' : '1px solid #ddd', borderRadius: '6px', color: isHidden ? 'var(--navy)' : '#666', cursor: 'pointer', fontWeight: 'bold' }}
                  >
                    {isHidden ? '👁️ Unhide' : '🙈 Hide Item'}
                  </button>
                  <button 
                    onClick={handleRealDelete}
                    style={{ flex: 1, padding: '10px', background: '#fff0f0', border: '1px solid #ffcccc', borderRadius: '6px', color: '#cc0000', cursor: 'pointer', fontWeight: 'bold' }}
                  >
                    🗑️ Delete Item
                  </button>
                </div>
